...
var keyy = window.localStorage.getItem('editkey')
var valuee = window.localStorage.getItem('editvalue')

db.collection('elective').doc(valuee).collection('information').doc(keyy).get()
.then((value) => {
  $('#years option[value='+ value.data().year +']').attr('selected',true);
  $('#ki option[value='+ value.data().semester +']').attr('selected',true);
  document.getElementById('areaedit').innerHTML = value.data().area;
  document.getElementById('lectureedit').innerHTML = value.data().lecture;
  document.getElementById('profedit').innerHTML = value.data().professor;
  changeprof(value.data().lecture,value.data().professor)
  if (value.data().team_project == 1){
    check.play(0)
    $('#teamcheck').text('있음')
    $('#boom').prop('checked', true)
  }
  if (value.data().announcement == 1){
    check2.play(0)
    $('#teamcheck2').text('있음')
    $('#boom2').prop('checked', true)
  }
  if (value.data().assignment == 0)  $("input:radio[name ='positive-change']:input[value='0']").attr("checked", true);
  else if (value.data().assignment == 1)  $("input:radio[name ='positive-change']:input[value='1']").attr("checked", true);
  else if (value.data().assignment == 2)  $("input:radio[name ='positive-change']:input[value='2']").attr("checked", true);
  else{ $("input:radio[name ='positive-change']:input[value='3']").attr("checked", true);}
  if (value.data().cost == 1){percent = 0.01;}
  else if (value.data().cost == 2){percent = 0.25;}
  else if (value.data().cost == 3){percent = 0.50;}
  else if (value.data().cost == 4){percent = 0.75;}
  else {percent = 0.99;}
  let rangeEl = document.getElementById('range')
  rangeEl.value = percent * 100
  rangeEl.addEventListener('input', moveThumb)
  drawCurve()
  drawThumb(percent)
  if (value.data().expenditure[0] == true )document.getElementById('project-contribute-1').checked = true;
  if (value.data().expenditure[1] == true )document.getElementById('project-contribute-2').checked = true;
  if (value.data().expenditure[2] == true )document.getElementById('project-contribute-3').checked = true;
  if (value.data().communication == 0) $("input:radio[name ='project-commu']:input[value='0']").attr("checked", true);
  if (value.data().communication == 1) $("input:radio[name ='project-commu']:input[value='1']").attr("checked", true);
  if (value.data().communication == 2) $("input:radio[name ='project-commu']:input[value='2']").attr("checked", true);
  if (value.data().tag1 == true )document.getElementById('project-tag-1').checked = true;
  if (value.data().tag2 == true )document.getElementById('project-tag-2').checked = true;
  if (value.data().tag3 == true )document.getElementById('project-tag-3').checked = true;
  if (value.data().tag4 == true )document.getElementById('project-tag-4').checked = true;
  if (value.data().tag5 == true )document.getElementById('project-tag-5').checked = true;
  if (value.data().tag6 == true )document.getElementById('project-tag-6').checked = true;
  $('.rating .star').removeClass('to_rate').addClass('rated')
  $('.rating .star')
    .parent()
    .find('.star:lt(' + (value.data().rating-1) + ')')
    .removeClass('to_rate')
    .addClass('rated')
  ...
  $('.rating .star')
    .parent()
    .find('.star:gt(' + (value.data().rating-1) + ')')
    .removeClass('no_to_rate')
    .removeClass('rated')
    document.getElementById('contact').value= value.data().content
    ...
    let refff = db.collection('elective').doc(valuee).collection('information')

    var rating = 0
    var count = 0
    var tagsCount = [0,0, 0, 0, 0, 0, 0]
    var expenditureCount = [0, 0, 0]
    var avgRating = 0.0
    var avgCost = 0.0
    var avgAssignment = 0.0
    var avgCommunication = 0.0
    var _tags = [0,tag1, tag2, tag3, tag4, tag5, tag6]
    var _expenditureSelections = [costreason1, costreason2, costreason3]
    db.collection('elective').doc(valuee).get()
    .then((valuee3) => {
        count = valuee3.data().count
        avgRating = valuee3.data().avg_rating
        avgAssignment = valuee3.data().avg_assignment
        avgCost = valuee3.data().avg_cost
        avgCommunication = valuee3.data().avg_communication
        if (_tags[1]) {
          tagsCount[1] = valuee3.data().tag1_count - 1
        } else {
          tagsCount[1] = valuee3.data().tag1_count
        }
        if (_tags[2]) {
          tagsCount[2] = valuee3.data().tag2_count - 1
        } else {
          tagsCount[2] = valuee3.data().tag2_count
        }
        if (_tags[3]) {
          tagsCount[3] = valuee3.data().tag3_count - 1
        } else {
          tagsCount[3] = valuee3.data().tag3_count
        }
        if (_tags[4]) {
          tagsCount[4] = valuee3.data().tag4_count - 1
        } else {
          tagsCount[4] = valuee3.data().tag4_count
        }
        if (_tags[5]) {
          tagsCount[5] = valuee3.data().tag5_count - 1
        } else {
          tagsCount[5] = valuee3.data().tag5_count
        }
        if (_tags[6]) {
          tagsCount[6] = valuee3.data().tag6_count - 1
        } else {
          tagsCount[6] = valuee3.data().tag6_count
        }

        for (var i = 0; i < _expenditureSelections.length; i++) {
          if (_expenditureSelections[i]) {
            expenditureCount[i] = valuee3.data().expenditureCount[i] - 1
          } else {
            expenditureCount[i] = valuee3.data().expenditureCount[i]
          }
        }

    }).then(() => {
      if (count-1 == 0){
        db.collection('elective').doc(valuee).set({
          count: count-1,
          avg_rating: 0,
          ...
          avg_communication: 0
            ...
            db.collection('elective').doc(valuee).set({
              ...
              tag6_count: tagsCount[5],
              expenditureCount: expenditureCount,
              avg_cost: ((count-1) * avgCost - Number(money)) /
              (count - 1),
              avg_assignment: ((count-1) * avgAssignment -
                Number(assi)) /
              (count - 1),
...
function acomplete () {
  ...
  // 후기 존재 여부 확인
  db.collection('elective').doc(valuee).get().then((value9) => {
      count = value9.data().count
      avgRating = value9.data().avg_rating
      avgAssignment = value9.data().avg_assignment
      avgCost = value9.data().avg_cost
      avgCommunication = value9.data().avg_communication

      if (_tags[1]) {
        tagsCount[1] = value9.data().tag1_count + 1
      } else {
        tagsCount[1] = value9.data().tag1_count
      }
      if (_tags[2]) {
        tagsCount[2] = value9.data().tag2_count + 1
      } else {
        tagsCount[2] = value9.data().tag2_count
      }
      if (_tags[3]) {
        tagsCount[3] = value9.data().tag3_count + 1
      } else {
        tagsCount[3] = value9.data().tag3_count
      }
      if (_tags[4]) {
        tagsCount[4] = value9.data().tag4_count + 1
      } else {
        tagsCount[4] = value9.data().tag4_count
      }
      if (_tags[5]) {
        tagsCount[5] = value9.data().tag5_count + 1
      } else {
        tagsCount[5] = value9.data().tag5_count
      }
      if (_tags[6]) {
        tagsCount[6] = value9.data().tag6_count + 1
      } else {
        tagsCount[6] = value9.data().tag6_count
      }

      for (var i = 0; i < _expenditureSelections.length; i++) {
        if (_expenditureSelections[i]) {
          expenditureCount[i] = value9.data().expenditureCount[i] + 1
        } else {
          expenditureCount[i] = value9.data().expenditureCount[i]
        }
      }
  }).then(() => {
    db.collection('elective').doc(valuee).set({
...
var lecount = 0
function changeprof (lec2,prof2) {

  for (i = 0; i <= 221; i++) {
    if (lectures[i][1] == lec2 && lectures[i][3] == prof2) {
